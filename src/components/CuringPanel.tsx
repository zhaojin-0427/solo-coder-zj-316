...
export default function CuringPanel() {
  ...
  const stepCards = useMemo(
    () => generateStepCards(layers, moldShape.areaMm2, stages),
    [layers, moldShape.areaMm2, stages]
  )

  const maxStageHours = Math.max(...estimate.stageDetails.map((d) => d.hours), 1)
  ...
  return (
    <>
      ...
      >
        <div className="flex flex-col items-center justify-center" style={{ minWidth: 140 }}>
          ...
          >
            ...
            <span className="text-sm font-normal ml-1" style={{ color: '#8B7355' }}>
              小时
            </span>
          </span>
          <span className="text-xs" style={{ color: '#8B7355' }}>
            {stages.length} 阶段 · {layers.length} 层 · {estimate.totalThickness.toFixed(1)} mm
          </span>
        </div>
        ...
        <div className="flex flex-col items-center justify-center" style={{ minWidth: 100 }}>
          ...
          <span
            ...
            style={{ color: getRiskColor(estimate.overallRisk) }}
          >
            {getRiskLabel(estimate.overallRisk)}
          </span>
        </div>
        ...
        >
          <div className="flex items-center gap-1.5">
            <ArrowRight size={12} style={{ color: '#C9A96E' }} />
            <span className="text-xs" style={{ color: '#C9A96E' }}>
              {estimate.nextSuggestion}
            </span>
          </div>
          {estimate.currentStageRisk && (
            <div className="flex items-center gap-1.5">
              <AlertTriangle size={12} style={{ color: getRiskColor(estimate.currentStageRisk.level) }} />
              <span
                className="text-xs"
                style={{ color: getRiskColor(estimate.currentStageRisk.level) }}
              >
                当前「{estimate.currentStageRisk.stageName}」
                {estimate.currentStageRisk.messages[0] || '无特殊风险'}
              </span>
            </div>
          )}
          <div className="flex-1 flex gap-2 overflow-x-auto scrollbar-thin py-1">
            {estimate.stageDetails.length > 0
              ? estimate.stageDetails.map((detail) => (
                  <div
                    key={detail.stageId}
                  ...
                  >
                    ...
                    <span className="text-xs" style={{ color: getRiskColor(detail.riskLevel) }}>
                      {getStageLabel(detail.stageType)} · {getRiskLabel(detail.riskLevel)}
                    </span>
                    <div
                    ...
                    >
                      <div
                        ...
                        style={{
                          height: '100%',
                          width: `${(detail.hours / maxStageHours) * 100}%`,
                          background: getRiskColor(detail.riskLevel),
                    ...
                    <div className="flex items-center gap-2 w-full justify-center">
                      ...
                      <span className="text-xs flex items-center gap-0.5" style={{ color: '#8B7355' }}>
                        <Droplets size={10} />
                        {detail.resinMl.toFixed(1)}ml
                      </span>
                      <span className="text-xs flex items-center gap-0.5" style={{ color: '#8B7355' }}>
                        <Ruler size={10} />
                        {detail.thickness.toFixed(1)}mm
                      </span>
                    </div>
                  </div>
                ))
        ...
        <div
          ...
          style={{ maxHeight: 90, minWidth: 240 }}
        >
      ...
      {showSteps && (
        ...
        >
          <div
            className="rounded-xl p-6 max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto scrollbar-thin"
          ...
          >
            ...
            <div className="space-y-4">
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-semibold text-sm" style={{ color: '#FFF8F0' }}>
                          {card.layerName}
                        </h4>
                        <span className="text-xs" style={{ color: '#8B7355' }}>
                    ...
                    <div className="text-right">
                      <div className="text-xs" style={{ color: '#87CEEB' }}>
                        胶量: {card.resinMl.toFixed(1)}ml
                      </div>
                      <div className="text-xs" style={{ color: '#C9A96E' }}>
                        固化: {card.curingHours.toFixed(1)}h
                      </div>
                      <div className="text-xs" style={{ color: '#8B7355' }}>
                        厚度: {card.thickness}mm
                      </div>
                    </div>
                  </div>
                  {card.elementCount > 0 && (
              ...
              >
                <h4 className="font-semibold text-sm mb-2" style={{ color: '#C9A96E' }}>
                  制作总结
                </h4>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div style={{ color: '#8B7355' }}>
                    模具类型: <span style={{ color: '#FFF8F0' }}>{moldShape.label}</span>
                  </div>
                  <div style={{ color: '#8B7355' }}>
                    总层数: <span style={{ color: '#FFF8F0' }}>{layers.length} 层</span>
                  </div>
                  <div style={{ color: '#8B7355' }}>
                    总厚度:{' '}
                    <span style={{ color: '#FFF8F0' }}>
                      {layers.reduce((s, l) => s + l.thickness, 0).toFixed(1)} mm
                    </span>
                  </div>
                  <div style={{ color: '#8B7355' }}>
                    总胶量: <span style={{ color: '#87CEEB' }}>{estimate.totalResinMl.toFixed(1)} ml</span>
                  </div>
                  <div style={{ color: '#8B7355' }}>
                    总固化时间: <span style={{ color: '#C9A96E' }}>{estimate.totalHours.toFixed(1)} 小时</span>
                  </div>
                  <div style={{ color: '#8B7355' }}>
                    材料总数:{' '}
                    <span style={{ color: '#FFF8F0' }}>
                      {layers.reduce((s, l) => s + l.elements.length, 0)} 个
                    </span>
                  </div>
                  <div style={{ color: '#8B7355' }}>
                    环境温度: <span style={{ color: '#F59E0B' }}>{ambientTemp}°C</span>
                  </div>
                  <div style={{ color: '#8B7355' }}>
                    整体风险:{' '}
                    <span style={{ color: getRiskColor(estimate.overallRisk) }}>
                      {getRiskLabel(estimate.overallRisk)}
                    </span>
                  </div>
                </div>
